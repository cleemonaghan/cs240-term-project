...
async function displayStudentPage() {
	...
	//otherwise, add the assignments they need to submit
		...
		let assignments = [10, 11, 12];
		for (let i = 0; i < assignments.length; i++) {
			tileHolder.appendChild(createTile("Name", assignments[i], 15, false));
		}
	}
	...
	//check how may graded assignments they have
	num = 3;
	//if the student has no assignments due, display that to screen
	if (num == 0) {
		//<h2>No assignments due</h2>
	...
	//otherwise, add the assignments they need to submit
		...
		let assignments = [10, 11, 12];
		for (let i = 0; i < assignments.length; i++) {
			tileHolder.appendChild(createTile("Name", assignments[i], 15, true));
		}
	}
	...
	//otherwise, add the assignments they need to submit
		...
		let assignments = [10, 11, 12];
		for (let i = 0; i < assignments.length; i++) {
			tileHolder.appendChild(createTile("Name", "-", 15, false));
		}
	}
...
function createTile(name, score, maxScore, pastDue) {
	...
	if (pastDue) {
		...
		tileScore.innerHTML = `${score}/${maxScore}`;
